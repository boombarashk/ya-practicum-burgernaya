...
function BurgerIngredients({ onHandleClick, onHandleScroll }) {
  ...
  useEffect(() => {
    ...
    TYPE_INGREDIENT.forEach((sectionParams) => {
      separateState[sectionParams.param] = ingredientsData.filter(
        (ingredient) => ingredient.type === sectionParams.param,
      );
    });
  ...
  return (
    <div className={`custom-scroll ${styles.inner}`} onScroll={onHandleScroll}>
      {TYPE_INGREDIENT.map((sectionParams) => {
        return (
          <section key={`section-${sectionParams.param}`}>
            <h2
              className={styles[sectionParams.param]}
