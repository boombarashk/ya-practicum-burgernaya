...
import {LOGIN_URL, REGISTER_URL, RESTORE_URL, RESET_URL, USER_URL, LOGOUT_URL } from '../../consts'
...
const fetchProfileRequest = (url) => createAsyncThunk(
    url,
...
export const getUserDetails = createAsyncThunk(
    USER_URL, 
    async () => fetchWithRefresh(USER_URL, {
        method: "GET"
    }).catch(e => {
...
export const patchUserDetails = createAsyncThunk(
    USER_URL, 
    async (body) => fetchWithRefresh(USER_URL, {
        method: "PATCH",
        body: JSON.stringify(body)
    }).catch(e => {
    console.log(e.message)
...
const profileSlice = createSlice({
    ...
    extraReducers: (builder)=> {
        ...
        builder.addMatcher((action) => action.type.match(new RegExp(`[${REGEXP_URLS}]/fulfilled`)), (state) => {
            state.loading = false
        })
        builder.addMatcher((action) => action.type.match(new RegExp(`[${REGEXP_URLS}]/pending`)), (state) => {
            state.error = initialProfileState.error
            state.loading = true
        })
        .addMatcher((action) => action.type.match(new RegExp(`[${REGEXP_URLS}]/rejected`)), (state, action) => {
            if (!action?.payload?.success){
